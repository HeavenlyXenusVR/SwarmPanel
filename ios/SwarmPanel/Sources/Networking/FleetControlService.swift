...
/// DashboardView's ownBotKey/ownSession computed properties do, using
/// AppState's persisted lastKnownGuildIdKey since AppState itself may not be
/// alive when an intent runs.
enum FleetControlService {
    static func ownSession() async throws -> (bot: DashboardBot, session: DashboardSession) {
        ...
        guard let guildId = UserDefaults.standard.string(forKey: AppState.lastKnownGuildIdKey), !guildId.isEmpty else {
